...
namespace engine
{
    // Handles Save Game, Load Game, Read monster data, Creating new player files
    class FileIO
    {
        ...
        internal static void BuildLoadablePlayersLists(out List<MenuItem> fileNames, out List<MenuItem> displayNames)
        {
        ...
        internal static void SavePlayer(string arg_0, Player player) // sub_47DFC
        {
                ...
                FileUtils.file_find(Path.Combine(Config.GetSavePath(), file_text) + ext_text) == true)
            {
                ...
                if (input_key == 'N')
                {
                    ...
                    {
                        file_text = TextRenderer.getUserInputString(8, 0, 10, "New file name: ");
                    }
                }
            }
    ...
    ClassId.unknown};
        ...
        internal static Player load_mob(int monster_id, bool exit)
        {
            ...
            if (decode_size == 0)
            {
                if (exit)
                {
                    TextRenderer.DisplayAndPause("Unable to load monster", 15);
