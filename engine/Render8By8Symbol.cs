...
namespace engine
{
    class Render8By8Symbol
    {
        internal static void Load8x8D(int symbolSet, int block_id)
        {
            if (symbolSet >= 0 && symbolSet < 5)
            {
                string text = "8x8d" + gbl.EclDaxFileNumber.ToString();
                gbl.symbol_8x8_set[symbolSet] = DrawPicture.LoadDax(13, 1, block_id, text);

                if (gbl.symbol_8x8_set[symbolSet] == null)
                {
                    Logger.LogAndExit("Unable to load {0} from 8x8D{1}", block_id, gbl.EclDaxFileNumber);
                }
        ...
        internal static void Put8x8Symbol(bool use_overlay, int symbol_id, int rowY, int colX)
        {
            ...
            if (gbl.symbol_8x8_set[symbol_set] != null)
            {
                ...
                if (use_overlay)
                {
                    DrawPicture.OverlayUnbounded(gbl.symbol_8x8_set[symbol_set], symbol_id, rowY, colX);
                }
                else
                {
                    ...
                    DrawPicture.draw_picture(gbl.cursor_bkup, rowY, colX, 0);
                }
            }
        }
    }
