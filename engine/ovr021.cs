...
namespace engine
{
    class ovr021
    {
        ...
        static void display_resting_time(int highlight_time) /* sub_58615 */
        {
            ...
            TextRenderer.displayString("Rest Time:", 0, 10, 17, 1);
            int col_x = 11;

            string text = format_time(gbl.timeToRest.field_8);
            TextRenderer.displayString(text, 0, colors[4], 0x11, col_x + 1);
            TextRenderer.displayString(":", 0, 10, 17, col_x + 3);
            col_x += 3;

            text = format_time(gbl.timeToRest.field_6);
            TextRenderer.displayString(text, 0, colors[3], 0x11, col_x + 1);
            TextRenderer.displayString(":", 0, 10, 17, col_x + 3);
            col_x += 3;

            text = format_time((gbl.timeToRest.field_4 * 10) + gbl.timeToRest.field_2);

            TextRenderer.displayString(text, 0, colors[2], 0x11, col_x + 1);
        }
        ...
        static bool resting_time_menu() /* sub_58751 */
        {
            ...
            do
            {
                ...
                input_key = KeyInputHandler.displayInput(out control_key, false, 1, gbl.defaultMenuColors, "Rest Days Hours Mins Add Subtract Exit", string.Empty);
        ...
        static void rest_heal(bool show_text) /* reset_heal */
        {
            ...
            if (gbl.rest_10_seconds >= (8 * 36))
            {
                ...
                TextRenderer.displayString("The Whole Party Is Healed", 0, 10, 19, 1);
        ...
        static int rest_memorize(ref bool findNext, Player player)
        {
            foreach (int id in player.spellList.LearningList())
            {
                ...
                else
                {
                    ...
                    Spells.DisplayCaseSpellText(id, "has memorized", player);
                    findNext = true;
                }
            }
                    ...
                    for (int spellIdx = 1; spellIdx < 4 && next_scribe_lvl == 0; spellIdx++)
                    {
                        if (item.getAffect(spellIdx) > (Affects)0x80)
                        {
                            ...
                            else
                            {
                                ...
                                player.LearnSpell((Classes.Spells)spellId);
                                Spells.remove_spell_from_scroll(spellId, item, player);

                                display_resting_time(0);

                                Spells.DisplayCaseSpellText(spellId, "has scribed", player);
                                findNext = true;
                            }
                        }
                    }
        ...
        internal static bool resting(bool interactive_resting) /* reseting */
        {
            ...
            {
                if (interactive_resting == true &&
                    KeyInputQueue.KEYPRESSED() == true)
                {
                    display_resting_time(0);

                    if (KeyInputHandler.yes_no(gbl.defaultMenuColors, "Stop Resting? ") == 'Y')
                    {
                        stop_resting = true;
                    }
                    else
